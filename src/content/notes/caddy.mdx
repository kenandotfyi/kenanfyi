---
draft: false
title: "Caddy configuration for MediaWiki"
...
<Code lang="apache" file="Caddyfile">
```nginx
your.domain{
    @title {
        not file {
    ...
    php_fastcgi unix//var/run/php/php-fpm.sock # serving php applications with Caddy
    file_server
}
```
</Code>

This configuration redirects the ugly MediaWiki URLs to a more structured **/wiki/Main_page** type. Further explanations are [here](https://www.mediawiki.org/wiki/Manual:Short_URL).

After that you need to edit the LocalSettings.php too:

<Code lang="php" file="LocalSettings.php">
```php
$wgScriptPath = "";
